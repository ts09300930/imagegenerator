...
# --- 設定（モデル名をVision対応の最新版に修正） ---
# xAIのVision APIで現在有効なIDです。
    ...
    try:
        ...
        if res.status_code == 200:
            if isinstance(json_res, dict) and "choices" in json_res:
                ...
                return f"API_ERROR: Unexpected structure: {str(json_res)[:100]}"
        else:
            # エラー詳細を表示（モデル名間違いならここで Model not found が出る）
            if isinstance(json_res, dict):
                msg = json_res.get('error', {}).get('message', str(json_res))
            else:
                msg = str(json_res)
...
# --- UI ---
st.title("Higgsfield Gen v9.0 (Verified Fix)")

# モデル名が万が一間違っている場合に備え、サイドバーで変更可能にしました
with st.sidebar:
    st.subheader("Debug Settings")
    GROK_MODEL = st.text_input("Grok Model ID", value="grok-2-vision-latest")
...
if mode == "📷 画像解析（アップロード）":
    ...
    if c2.button(f"🎲 {gen_count}件を自動生成", use_container_width=True):
        # シチュエーション生成は画像不要なのでテキスト用モデルでも動くが、統一してGROK_MODELを使用
        ...
        with st.container():
            ...
            if item["type"] == "image":
                ...
                with st.spinner(f"画像 {i+1} を解析中..."):
                    current_ctx = call_grok_api([{"role":"user","content":[{"type":"text","text":"Describe background and outfit."},{"type":"image_url","image_url":{"url":f"data:image/jpeg;base64,{img_b64}"}}]}])
            else:
            ...
            with st.spinner(f"プロンプト {i+1} を合成中..."):
                ...
                extras = [sex_map[sex_level], quality]
                ...
                instruction = (
                    ...
                    f"Style: {', '.join(extras)}.\n"
                    "Task: Create a detailed English prompt starting with 'A photorealistic shot of...'"
                )

                final_p = call_grok_api([{"role":"user","content": instruction}])
