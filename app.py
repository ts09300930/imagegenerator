...
# ====================== UI ======================
st.title("Higgsfield Gen v10.1 (リアル写真モード・露出制御対応)")
...
safe_mode = st.checkbox("✅ 露出を完全に抑える（露出度0 / 谷間・透け・乳首などを排除）", value=False)
    ...
    for idx, item in enumerate(targets):
        with st.container():
            ...
            with st.spinner("最終プロンプト合成中..."):
                ...
                if nipple_poke and not safe_mode: adds.append("visible nipples poking through fabric")
                ...
                # 露出度0の安全指示（最優先）
                safe_instruction = (
                    "modest clothing, no cleavage, no underboob, no see-through, no visible nipples, "
                    "no upskirt, no erotic exposure, conservative outfit, fully covered, safe for work"
                ) if safe_mode else ""

                system_content = (
                    f"High-end Prompt Engineer for AI image generation. "
                    f"Sexiness level: {sex_level if not safe_mode else 0}. "
                    f"{bust_ins}. "
                    f"{', '.join(adds) if adds else ''}. "
                    f"{style_instruction}. "
                    f"{safe_instruction}. "
...
st.caption("Higgsfield Gen v10.1 — リアル写真モード & 露出度0制御 搭載版")
